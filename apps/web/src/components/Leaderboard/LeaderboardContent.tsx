...
type CompetitionStatusTag = {
  status: "ONGOING" | "UPCOMING" | "ENDED";
};

// Helper components
const StatusTag = ({ status }: CompetitionStatusTag) => {
  const getTagStyles = () => {
    switch (status) {
      case "ONGOING":
...
export default function LeaderboardContent() {
  ...
  // Organized competition data for the dropdown
  const organizedCompetitions = useMemo(() => {
    if (!competitionData) return { ongoing: [], upcoming: [], past: [] };
    return organizeCompetitions(competitionData);
  }, [competitionData]);
                ...
                {competitionNameLoading ? (
                  ...
                  <>
                    {renderCompetitionSection(
                      organizedCompetitions.ongoing,
                      "Ongoing Competitions"
                    )}
                    {renderCompetitionSection(
                      organizedCompetitions.upcoming,
                      "Upcoming Competitions"
                    )}
                    {renderCompetitionSection(
                      organizedCompetitions.past,
                      "Past Competitions"
                    )}
                  </>
                )}
        ...
        <Table.Root>
          ...
          <Table.Body>
            {isLoading ? (
              ...
              leaderboardData.map((entry: LeaderboardResult, index: number) => (
